...
locals {
  ingress = {
    ...
    ping_100 = { from =   8, to=8    , protocol = "icmp", cidr = "100.99.0.0/16"  }
    ssh_100  = { from =  22, to=22   , protocol = "tcp" , cidr = "100.99.0.0/16"  }
...
resource "aws_security_group_rule" "ingress" {
  for_each = merge(local.ingress, var.ingress)
...
resource "aws_security_group_rule" "egress" {
  for_each = merge(local.egress, var.egress)
